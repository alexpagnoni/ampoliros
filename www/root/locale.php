 ...
 *
 */
// $Id: locale.php,v 1.20 2004-07-08 15:04:25 alex Exp $
...
$amp_locale = new Locale('amp_root_locale', $gEnv['root']['locale']['language']);
...
function pass_setlanguage($eventData) {
    ...
    $amp_config -> SetValue('AMP_LANG', $eventData['language']);

    $log -> LogEvent('Ampoliros', 'Changed Ampoliros root language', LOGGER_NOTICE);
...
function main_default($eventData) {
    ...
    $hui_locale_grid -> AddChild(new HuiComboBox('country', array('disp' => 'pass', 'elements' => $countries, 'default' => ($selected_country ? $selected_country : AMP_COUNTRY))), 0, 1);
...
function main_language($eventData) {
    ...
    $loc_country = new LocaleCountry(AMP_COUNTRY);
    ...
    $hui_locale_grid -> AddChild(new HuiComboBox('language', array('disp' => 'pass', 'elements' => $languages, 'default' => ($selected_language ? $selected_language : AMP_LANG))), 0, 1);
...
function main_help($eventData) {
    ...
    $hui_mainframe -> AddChild(new HuiHelpNode('locale_help', array('node' => 'ampoliros.root.locale.'.$eventData['node'], 'language' => AMP_LANG)));
}
